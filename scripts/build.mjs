...
import * as $ from 'hast-util-select'
 ...
 */
function parseArgs() {
  const [, script, outputDir] = process.argv
  if (!outputDir) {
    console.error(`Usage: node ${script} output/dir`)
    process.exit(1)
  }
  return { outputDir }
}
 ...
 */
async function write(name, id, data, type) {
  ...
  await fs.promises.writeFile(
    path.resolve(resolvedDir, `${name}.ts`),
    Buffer.from(
      type && type !== 'unknown'
        ? `import { ${type} } from '../types'

export const ${id}: ${type}[] = ${json}
`
        : `export const ${id} = ${json} as const\n`
    )
  )
}
...
Promise.resolve()
  ...
  .then((doc) => {
    ...
    rows.forEach(
      ...
      function (row) {
        ...
        let [name, alpha2, alpha3, numeric] = cells.map(cleanNode)
  ...
  .then((doc) => {
    ...
    entries.forEach((entry) => {
      let { state, note, alpha2 } = entry
      const record = assigned.find((d) => d.alpha2 === alpha2)
      let { name, alpha3, numeric } = record || {}
  ...
  .then(() => {
    ...
    const codes = iso31661.filter((d) => d.state === 'assigned' && d.alpha2 === 'GB')
  ...
  .then((doc) => {
    ...
    rows.forEach(function (row) {
      ...
      let [alpha4, name, before, , after] = cells.map(cleanNode)
      /** @type {string} */
      let kind = null
      let lastIndex = 0
      ...
      for (key in types) {
        ...
        if (match) {
          kind = key
          after = after.slice(0, match.index) + after.slice(match.index + match[0].length)
  ...
  .then(() => {
    ...
    return Promise.all([
      write('1', 'iso3166_1', iso31661Assigned, 'Iso3166_1Assigned'),
      write('1-reserved', 'iso3166_1Reserved', iso31661Reserved, 'Iso3166_1Reserved'),
      write('1-a2-to-1-a3', 'iso3166_1Alpha2ToAlpha3', a2ToA3),
      write('1-a2-to-1-n', 'iso3166_1Alpha2ToNumeric', a2ToN),
      write('1-a3-to-1-a2', 'iso3166_1Alpha3ToAlpha2', a3ToA2),
      write('1-n-to-1-a2', 'iso3166_1NumericToAlpha2', nToA2),
      write('2', 'iso3166_2', iso31662, 'Iso3166_2'),
      write('3', 'iso3166_3', iso31663, 'Iso3166_3'),
    ])
  })
  .catch(
